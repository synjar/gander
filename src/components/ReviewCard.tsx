  ...
  return (
    <article className="border-b border-stone-100 py-5 last:border-0">
      <div className="flex items-center gap-3">
        <Link to={`/u/${r.authorId}`} className="shrink-0">
          <Avatar name={r.authorName} src={r.authorAvatar} size={44} className="hover:ring-2 hover:ring-brand-300 transition" />
        </Link>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            <Link to={`/u/${r.authorId}`} className="font-semibold text-stone-900 hover:text-brand-600">
              {r.authorName}
            </Link>
